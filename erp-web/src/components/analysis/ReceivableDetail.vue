<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>应收账款明细表</el-breadcrumb-item>
    </el-breadcrumb>
    ...
    <el-card>
      ...
      <el-table :data="list"
        ...
        <el-table-column label="增加应收款"
                         prop="increasedAmount"></el-table-column>
        <el-table-column label="支付应收款"
...
export default {
  ...
  methods: {
    ...
    // 获取列表
    async getList() {
      const { data: result } = await this.$http.post('/analysis/receivable/detailList', this.params)
    ...
    getSummaries(param) {
      ...
      columns.forEach((column, index) => {
        if (index === 0) {
          sums[index] = '合计'
          return
        } else if (column.label !== '增加应收款' && column.label !== '支付应收款' && column.label !== '应收款余额') {
          sums[index] = ''
          return
        }
