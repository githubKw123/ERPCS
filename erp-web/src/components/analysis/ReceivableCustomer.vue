<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>客户对账单</el-breadcrumb-item>
    </el-breadcrumb>
...
export default {
  ...
  methods: {
    ...
    // 获取列表
    async getList() {
      const { data: result } = await this.$http.post('/analysis/receivable/customerList', this.params)
      if (!result.success) return this.$message.error(result.message)

      this.list = result.data.receivableList
    },
    // 计算合计
    getSummaries(param) {
      ...
      columns.forEach((column, index) => {
        if (index === 0) {
          sums[index] = '合计'
          return
        } else if (column.label !== '销售金额' && column.label !== '整单折扣额' && column.label !== '应收金额' && column.label !== '实际收款金额' && column.label !== '应收款余额') {
          sums[index] = ''
          return
        }
    ...
    rowClick(row) {
      if (row.businessType === 'sell') {
        ...
        this.$router.push({
          ...
          query: { collectionId: row.businessId }
        })
      }
    }
  }
