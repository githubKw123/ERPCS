<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>供应商对账单</el-breadcrumb-item>
    </el-breadcrumb>
...
export default {
  ...
  mounted() {
    ...
    if (params !== undefined) {
      if (params.supplierId !== undefined) {
        this.params.supplierId = params.supplierId
      }
  ...
  methods: {
    ...
    // 获取列表
    async getList() {
      const { data: result } = await this.$http.post('/analysis/payable/supplierList', this.params)
    ...
    getSummaries(param) {
      ...
      columns.forEach((column, index) => {
        if (index === 0) {
          sums[index] = '合计'
          return
        } else if (column.label !== '采购金额' && column.label !== '整单折扣额' && column.label !== '应付金额' && column.label !== '实际付款金额' && column.label !== '应收款余额') {
          sums[index] = ''
          return
        }
