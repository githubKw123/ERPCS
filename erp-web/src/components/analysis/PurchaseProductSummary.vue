<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>采购汇总表（按商品）</el-breadcrumb-item>
    </el-breadcrumb>
...
export default {
  ...
  methods: {
    ...
    // 获取列表
    async getList() {
      const { data: result } = await this.$http.post('/analysis/purchase/productSummaryList', this.params)
    ...
    rowClick(row) {
      this.$router.push({
        ...
        query: { params: { ...this.params, productId: row.productId } }
      })
    }
  }
}
