<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>现金银行报表</el-breadcrumb-item>
    </el-breadcrumb>
...
export default {
  ...
  methods: {
    ...
    // 获取列表
    async getList() {
      const { data: result } = await this.$http.post('/analysis/account/detailList', this.params)
    ...
    getSummaries(param) {
      ...
      columns.forEach((column, index) => {
        if (index === 0) {
          sums[index] = '合计'
          return
        } else if (column.label !== '收入' && column.label !== '支出' && column.label !== '账户余额') {
          sums[index] = ''
          return
        }
    ...
    rowClick(row) {
      ...
      } else if (row.businessType === 'income') {
        this.$router.push({
          path: '/income/save',
