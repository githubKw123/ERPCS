<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>采购汇总表（按供应商）</el-breadcrumb-item>
    </el-breadcrumb>
...
export default {
  ...
  methods: {
    ...
    // 获取列表
    async getList() {
      const { data: result } = await this.$http.post('/analysis/purchase/supplierSummaryList', this.params)
    ...
    getSummaries(param) {
      ...
      columns.forEach((column, index) => {
        if (index === 0) {
          sums[index] = '合计'
          return
        } else if (column.label !== '数量' && column.label !== '采购金额') {
          sums[index] = ''
          return
        }
    ...
    rowClick(row) {
      this.$router.push({
        ...
        query: { params: { ...this.params, supplierId: row.supplierId } }
      })
    }
  }
}
