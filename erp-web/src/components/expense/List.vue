<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>其他支出单列表</el-breadcrumb-item>
    </el-breadcrumb>
    ...
    <el-card>
      ...
      <el-row :gutter="20"
        ...
        <el-col :span="8">
          <el-input placeholder="请输入单据号/供应商名称/备注查询"
          ...
          <template slot-scope="scope">
            ...
            <el-tooltip effect="dark"
              ...
              <el-button type="danger"
                         icon="el-icon-delete"
                         size="mini"
                         @click="deleteExpense(scope.row.id)"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    // 获取支出单分页列表
    async getPage() {
      const { data: result } = await this.$http.post('/expense/page', this.params)
      if (!result.success) return this.$message.error(result.message)

      this.list = result.data.expensePage.records

      this.total = result.data.expensePage.total
    },
    // 监听 pagesize 改变的事件
    ...
    // 新增按钮
    goAddPage() {
      this.$router.push('/expense/save')
    },
    // 修改按钮
    goEditPage(expenseId) {
      this.$router.push({
        path: '/expense/save',
        query: { expenseId }
      })
    ...
    async deleteExpense(expenseId) {
      // 弹框询问用户是否删除数据
      const confirmResult = await this.$confirm('此操作将永久删除该支出单，是否继续?', '提示', {
      ...
      const { data: result } = await this.$http.post('/expense/delete', { expenseId })

      if (!result.success) {
        return this.$message.error('删除支出单失败！')
      }

      this.$message.success('删除支出单成功！')
      this.getPage()
    }
  }
}
