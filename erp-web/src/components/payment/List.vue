<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>付款单列表</el-breadcrumb-item>
    </el-breadcrumb>
          ...
          <template slot-scope="scope">
            ...
            <el-tooltip effect="dark"
              ...
              <el-button type="danger"
                         icon="el-icon-delete"
                         size="mini"
                         @click="deletePayment(scope.row.id)"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    // 获取付款单分页列表
    async getPage() {
      const { data: result } = await this.$http.post('/payment/page', this.params)
      if (!result.success) return this.$message.error(result.message)

      this.list = result.data.paymentPage.records

      this.total = result.data.paymentPage.total
    },
    // 监听 pagesize 改变的事件
    ...
    // 新增按钮
    goAddPage() {
      this.$router.push('/payment/save')
    },
    // 修改按钮
    goEditPage(paymentId) {
      this.$router.push({
        path: '/payment/save',
        query: { paymentId }
      })
    ...
    async deletePayment(paymentId) {
      // 弹框询问用户是否删除数据
      const confirmResult = await this.$confirm('此操作将永久删除该付款单，是否继续?', '提示', {
      ...
      const { data: result } = await this.$http.post('/payment/delete', { paymentId })

      if (!result.success) {
        return this.$message.error('删除付款单失败！')
      }
