<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>收款单列表</el-breadcrumb-item>
    </el-breadcrumb>
    ...
    <el-card>
      ...
      <el-table :data="list"
        ...
        <el-table-column label="销货单位"
                         prop="customerName"></el-table-column>
        <el-table-column label="收款金额"
                         prop="collectAmount"></el-table-column>
          ...
          <template slot-scope="scope">
            ...
            <el-tooltip effect="dark"
              ...
              <el-button type="danger"
                         icon="el-icon-delete"
                         size="mini"
                         @click="deleteCollection(scope.row.id)"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    // 获取收款单分页列表
    async getPage() {
      const { data: result } = await this.$http.post('/collection/page', this.params)
      if (!result.success) return this.$message.error(result.message)

      this.list = result.data.collectionPage.records

      this.total = result.data.collectionPage.total
    },
    // 监听 pagesize 改变的事件
    ...
    // 新增按钮
    goAddPage() {
      this.$router.push('/collection/save')
    },
    // 修改按钮
    goEditPage(collectionId) {
      this.$router.push({
    ...
    async deleteCollection(collectionId) {
      // 弹框询问用户是否删除数据
      const confirmResult = await this.$confirm('此操作将永久删除该收款单，是否继续?', '提示', {
      ...
      const { data: result } = await this.$http.post('/collection/delete', { collectionId })

      if (!result.success) {
        return this.$message.error('删除收款单失败！')
      }
