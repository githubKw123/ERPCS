<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>其他出库单列表</el-breadcrumb-item>
    </el-breadcrumb>
          ...
          <template slot-scope="scope">
            ...
            <el-tooltip effect="dark"
              ...
              <el-button type="danger"
                         icon="el-icon-delete"
                         size="mini"
                         @click="deleteCheckout(scope.row.id)"></el-button>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  methods: {
    ...
    // 获取出库单分页列表
    async getPage() {
      const { data: result } = await this.$http.post('/checkout/page', this.params)
      if (!result.success) return this.$message.error(result.message)

      this.list = result.data.checkoutPage.records

      this.total = result.data.checkoutPage.total
    },
    // 监听 pagesize 改变的事件
    ...
    // 新增按钮
    goAddPage() {
      this.$router.push('/checkout/save')
    },
    // 修改按钮
    goEditPage(checkoutId) {
      this.$router.push({
        path: '/checkout/save',
        query: { checkoutId }
      })
    ...
    async deleteCheckout(checkoutId) {
      ...
      const { data: result } = await this.$http.post('/checkout/delete', { checkoutId })

      if (!result.success) {
        return this.$message.error('删除出库单失败！')
      }
